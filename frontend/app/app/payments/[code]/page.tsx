...
/**
 * Payment Detail page — CANONICAL detail route for a single Payment record.
 *
 * Contract: ADR-OPS-001 (UI-C1.2 Operations Center reconciliation, §6/§21) —
 * canonical Payments detail is `/app/payments/[code]`; the historical
 * `/app/finance/payments/[id]` is a compatibility redirect kept for bookmarks.
 *
 * URL identifier: Payment business code (`PAY-*`) — unique, immutable.
 * Backend endpoint: GET /finance/payments/:code (lookup by business code).
 * Display identifier: `referenceNumber` (MKT-PAY-*) per the project-wide
 * commercial reference presentation contract (Pre-Step 3.12).
 */
...
export default function PaymentDetailPage() {
  ...
  const code = params.code as string; // route param is [code] = Payment business code
