...
function FlightCard({ card }: { card: PublicProductCard }) {
...
export default function Flights() {
  const locale = useLocale();
  const [flights, setFlights] = useState<PublicProductCard[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState(false);

  useEffect(() => {
    let alive = true;
    // Query for FLIGHT type products. Currently 0 FLIGHT products exist in
    // the demo seed — the section will hide itself (return null) when empty.
    // This is correct behavior per spec: no fake data, no broken UI.
    void publicApi
      .listProducts({ sort: "newest", pageSize: 50 })
      .then((r) => {
        if (alive) {
          const flightsOnly = r.items.filter((item) => item.type === "FLIGHT").slice(0, 6);
          setFlights(flightsOnly);
          setLoading(false);
        }
      })
      .catch(() => {
        ...
        <div className="mb-8 flex items-end justify-between sm:mb-10">
          <div>
            <h2 className="font-serif text-2xl font-bold text-white sm:text-3xl">
              {t("marketplace.flights_title", locale)}
            </h2>
            <p className="mt-2 text-sm text-neutral-400">
              {t("marketplace.flights_subtitle", locale)}
            </p>
          </div>
          <Link
          ...
          >
            {t("marketplace.all_flights", locale)}
