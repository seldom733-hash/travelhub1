...
export default function Tours() {
  ...
  useEffect(() => {
    let alive = true;
    // Fetch published TOUR products. Skip the first 6 (shown in Hot Tours)
    // to avoid duplication. This uses the same backend query as Hot Tours
    // but with client-side offset for separation.
    void publicApi
      .listProducts({ sort: "newest", pageSize: 50 })
      .then((r) => {
        if (alive) {
          const toursOnly = r.items
            .filter((item) => item.type === "TOUR")
            .slice(6, 12); // Skip first 6 (Hot Tours), take next 6
          setTours(toursOnly);
          setLoading(false);
        }
      })
    ...
    <section className="bg-dark py-16 sm:py-20">
      <div className="mx-auto max-w-[1400px] px-6">
        ...
        <div className="mb-8 flex items-end justify-between sm:mb-10">
          <div>
            <h2 className="font-serif text-2xl font-bold text-white sm:text-3xl">
              {t("marketplace.tours_title", locale)}
            </h2>
            <p className="mt-2 text-sm text-neutral-400">
              {t("marketplace.tours_subtitle", locale)}
            </p>
          </div>
          <Link
          ...
          >
            {t("marketplace.all_tours", locale)}
        ...
        {!loading && tours.length > 0 && (
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
            {tours.map((tour) => (
              <TourCard key={tour.id} card={tour} />
            ))}
          </div>
        )}
