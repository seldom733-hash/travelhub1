...
## FINAL STRICT IMPLEMENTATION PROMPT
...
Отдельно установи по фактическому repository, был ли Step 1.15A `Business Event Temporal Contract` реализован и принят. Не считай его завершённым только по наличию упоминания в roadmap или другом prompt. Если contract существует в коде, ADR и tests — включи его в regression. Если нет — зафиксируй фактическое состояние, но не реализовывай Step 1.15A скрытно внутри Step 1.17.
...
## 5.1 Authentication и session boundaries

Дополнительно проверь:

- anonymous не получает authenticated resources;
- public endpoints действительно работают анонимно;
- наличие authenticated session на public request не расширяет public projection и не ослабляет visibility policy;
- `/account/*` корректно BUYER-gated;
- `/partner/*` корректно PARTNER-gated;
- `/app/*` недоступен BUYER/PARTNER из-за broad permission или route bug;
- login/deep-link `next`/`returnTo` не создаёт open redirect;
- logout/session invalidation не оставляет доступ к protected API;
- 401/403/neutral 404 используются последовательно;
- cookie/token/password/Authorization header не попадают в logs, errors, AuditLog или behavioral payload.

Не вводи искусственное правило «public frontend никогда не отправляет Authorization». Проверяется результат: public contract остаётся одинаково безопасным с заголовком и без него, если текущая архитектура допускает optional authentication.
...
## 11.1 Seller identity и anti-disintermediation

Проверь отдельно:

- Marketplace seller projection формируется только через утверждённую `PublicSellerProfile` policy;
- raw CRM Partner и Storefront business identity не подменяют Marketplace identity;
- contact data не проходит через Product description/attributes/media metadata/seller projection там, где действуют существующие validators/policies;
- Storefront structured contacts разрешены только в Storefront context;
- изменение или скрытие seller profile не оставляет stale public leakage;
- альтернативные RU/AZ/EN labels/fields не обходят contact policy;
- behavioral payload не получает contact values.

Не расширяй anti-disintermediation новым AI/moderation продуктом.

## 11.2 Storefront-specific hardening

Проверь:

- explicit provisioning без автоматического создания Storefront всем Partner;
- slug uniqueness, concurrency и reserved words;
- owner-only preview и нейтральное поведение для чужого preview;
- preview не становится public publication;
- public SEO/canonical metadata не раскрывает DRAFT/INACTIVE/non-entitled состояние;
- structured contact URL schemes безопасны;
- Storefront media принадлежит правильному tenant и не протекает между Partner;
- behavioral events не записываются для preview, если это является текущим contract;
- повторные команды существующих Storefront/entitlement transitions не дают 500 или inconsistent state.

Не создавай отсутствующие entitlement transitions, audit workflow или Billing authority ради выполнения этого пункта. Проверяй только фактически существующий contract; отсутствие будущего flow документируй как limitation/deferred decision.
...
Не устанавливай абсолютное требование «behavioral endpoint не принимает Authorization». Допустимы anonymous и authenticated requests, если текущий contract это предусматривает. Обязательно доказать корректное разделение actor/session, отсутствие authority spoofing и одинаковое применение resource visibility policy.
...
# 16A. XSS, injection и unsafe URL

Проведи targeted repo-wide проверку:

- `dangerouslySetInnerHTML` и другие raw HTML render paths;
- stored text из Product/Storefront/Communication/moderation feedback;
- `javascript:`/неразрешённые `data:` и другие опасные URL schemes;
- внешние ссылки и `noopener noreferrer` там, где это требуется;
- Prisma raw SQL и обязательную parameterization;
- search/filter/sort injection;
- slug/path/header/log injection;
- upload MIME, magic bytes/decode и SVG policy;
- exception/log output на control characters и raw payload.

Не превращай plain-text contracts в rich HTML и не добавляй новый sanitizer pipeline без фактической необходимости.

---

# 16B. Performance, N+1 и индексы

Без преждевременной оптимизации проверь текущие regression-risk paths:

- public catalog list/search/category/PDP;
- Storefront product list/PDP;
- seller projection и availability loading;
- Partner/Buyer lists;
- moderation queues;
- Communication lists;
- behavioral ingestion.

Ищи unbounded lists, очевидные N+1, count/list predicate divergence, загрузку больших raw relations и отсутствие индекса под реальный текущий predicate/order. Исправляй только измеримый или явно доказуемый локальный дефект. Cache layer, warehouse и крупный query redesign остаются вне scope.
...
## 17.1 Localization boundary

Не реализовывай multilingual business content или AI translation. Проверь только текущие обязательства:

- system UI RU/AZ/EN не ломает routes или DTO;
- geography/status labels используют canonical codes;
- locale switch не меняет object scope/tenant/visibility;
- отсутствие перевода business content не заполняется выдуманным переводом;
- локализованные альтернативные поля не обходят contact/XSS validation.
...
Дополнительно включи, если соответствующие paths реально существуют:

- Storefront create/slug race;
- repeated/concurrent lifecycle command;
- Outbox/Inbox/event dedup;
- behavioral `eventId` dedup и occurredAt skew bounds;
- ALS isolation при параллельных requests;
- open redirect и unsafe URL;
- staged/private media probing;
- malformed filters/sort keys без raw ORM/SQL paths.
...
## 19.1 Seed, startup и reconciliation audit

Проверь каждый production-reachable `onModuleInit`, seed/reconciliation path и startup side effect. Не должно происходить:

- автоматическое создание Storefront всем legacy Partner;
- activation entitlement по умолчанию без authority;
- synthetic Communication/behavioral/business records;
- duplicate roles/permissions/channels;
- fake historical timestamps;
- destructive cleanup;
- скрытая зависимость production boot от test/dev data.

Seeds/reconciliation должны быть idempotent и безопасны. Не запускай production-mutating seed против общей dev/production-like БД без изоляции.

## 19.2 Direct database writes и invariant bypass

Repo-wide найди прямые Prisma writes/scripts и классифицируй их как production path, migration/backfill, seed, test fixture или legacy/offline script. Production write, обходящий ownership/lifecycle/history/event contract, является finding. Не переписывай без влияния на runtime legacy/offline scripts только ради косметической чистоты.
...
# 22. Definition of Done для завершения implementation
...
Это не критерии `APPROVAL`: implementation-agent не имеет права присваивать APPROVAL. Он может только подтвердить завершение implementation и ожидание независимого Strict Review.
