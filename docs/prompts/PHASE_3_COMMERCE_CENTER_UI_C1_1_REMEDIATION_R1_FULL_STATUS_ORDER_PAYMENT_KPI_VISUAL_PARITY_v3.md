...
# 10A. KPI TYPOGRAPHY & CONTENT LAYOUT CONTRACT — HARD GATE

Previous UI-C1.1 incorrectly treated matching border/radius classes as sufficient KPI unification.

This remediation must unify not only KPI card container geometry, but also the **internal textual hierarchy and content layout** across:
...
Observed inconsistency to eliminate:
...
Canonical KPI content hierarchy for ALL 3 Commerce registries:
...
same label font-weight
same label line-height
same label casing
same value font-size
same value font-weight
same value line-height
same internal gap
same horizontal padding
same vertical padding
same min-height strategy
same label area behavior
same wrapping rules
...
business label text
numeric value
semantic status/payment color/accent where applicable
```

---

# 10B. NO KPI TYPOGRAPHY DRIFT

Forbidden differences between Requests / Orders / Bookings:

```text
one registry uses uppercase labels and another sentence case
one registry centers text and another left-aligns
one registry puts value first and another label first
one registry uses different label size/weight
one registry uses icon + text while others do not
one registry truncates labels with ellipsis while others wrap
one registry uses different content spacing
```

Do not report KPI visual parity if any of these remain.

---

# 10C. KPI LABEL CASING

Canonical visible casing:

```text
normal localized business label
```
...
Do NOT force uppercase through:

```text
uppercase
text-transform
all-caps localization strings
```

unless a specific global design token explicitly requires uppercase for ALL 3 registries. For this remediation the preferred contract is **sentence/title-like localized business casing, not ALL CAPS**.
...
for both label and value.

Do not center Requests while left-aligning Orders.

---

# 10E. KPI DECORATIVE ICON POLICY

Current Orders KPI cards include decorative icons while Requests do not.

For this remediation:

```text
REMOVE decorative KPI icons
```

from the canonical Commerce registry KPI component unless icons are demonstrated as a semantic requirement for EVERY corresponding KPI across Requests, Orders, and Bookings.

Do not invent 10–15 status icons merely to preserve old Orders decoration.

Status meaning should primarily come from:
...
# 10F. LONG LABEL / WRAPPING CONTRACT

Because all canonical statuses are now shown as cards, long labels are expected.

Mandatory:

```text
NO ellipsis truncation for canonical KPI labels
NO hidden full status name
NO "ГОТОВЫ К БРОНИР..."
```

Labels must safely wrap to the required number of lines.

Card grid/card height strategy must keep rows visually coherent.

If necessary use:

```text
min-height for label zone
consistent line-clamp = none
word wrapping
responsive columns
```

Do not shrink typography below readable Enterprise SaaS standards just to avoid wrapping.

---

# 10G. ONE REAL SHARED KPI COMPONENT

All three registries must consume the same actual component or the same single source of rendered markup/styles:
...
```text
Requests → same CommerceKpiCard
Orders   → same CommerceKpiCard
Bookings → same CommerceKpiCard
```

A component that exists in the repo but is not used by all three does NOT satisfy this gate.

The canonical component must own:
...
Page files may provide only data/semantic props, not duplicate markup or override the content hierarchy.

---

# 10H. KPI DOM / VISUAL PARITY EVIDENCE

Report exact rendered/component structure for each registry.

Required matrix:

| Property | Requests | Orders | Bookings | Canonical | Result |
|---|---|---|---|---|---|
| Shared component | | | | CommerceKpiCard | |
| DOM hierarchy | | | | Label → Value | |
| Label position | | | | Top | |
| Value position | | | | Below label | |
| Alignment | | | | Left | |
| Label casing | | | | Normal localized casing | |
| Label font-size | | | | shared token | |
| Label weight | | | | shared token | |
| Value font-size | | | | shared token | |
| Value weight | | | | shared token | |
| Internal gap | | | | shared token | |
| Card padding | | | | shared token | |
| Min-height | | | | shared strategy | |
| Wrapping | | | | wrap, no ellipsis | |
| Decorative icon | | | | absent | |
| Selected state | | | | shared token | |
| Focus state | | | | shared token | |

Any mismatch = FAIL.

---

# 10I. KPI VISUAL SCREENSHOT HARD GATE

Screenshots must prove side-by-side equivalent visual language for:
...
Reviewer must be able to confirm visually:

```text
same label-above-value hierarchy
same alignment
same casing
same typography
same card density
same wrapping
same spacing
same border/radius grammar
```

If only CSS class inventory is provided without actual browser visual proof:
...
10. KPI Typography & Content Layout Contract
11. KPI Shared Component Parity
12. Requests KPI Implementation
13. Orders Lifecycle KPI Implementation
14. Orders Payment KPI Implementation
15. Bookings KPI Implementation
16. KPI Drill-down
17. KPI Reconciliation
18. Search/Filter Canonical Contract
19. Live Search Implementation
20. Detail Card Completion
21. Registry Visual Parity
22. Responsive Qualification
23. i18n
24. Accessibility
25. Security/Scope Preservation
26. Tests
27. Regression/Build Results
28. Browser Qualification
29. Screenshot Evidence
30. Lifecycle Reconciliation Table
31. Payment Reconciliation Table
32. Search/Filter Qualification Table
33. Detail Card Parity Table
34. Status Naming Reconciliation Matrix
35. KPI DOM / Visual Parity Matrix
36. File Change Inventory
...
| One actual shared CommerceKpiCard used by Requests | | |
| One actual shared CommerceKpiCard used by Orders | | |
| One actual shared CommerceKpiCard used by Bookings | | |
| KPI DOM hierarchy identical across all 3 registries | | |
| KPI label above value across all 3 | | |
| KPI alignment identical across all 3 | | |
| KPI label casing identical across all 3 | | |
| KPI label typography identical across all 3 | | |
| KPI value typography identical across all 3 | | |
| KPI internal spacing identical across all 3 | | |
| KPI min-height strategy identical across all 3 | | |
| KPI labels wrap without ellipsis | | |
| Decorative KPI icons absent | | |
| KPI selected state identical across all 3 | | |
| KPI focus state identical across all 3 | | |
| KPI visual screenshot parity proven | | |
| KPI DOM / Visual Parity Matrix complete | | |
