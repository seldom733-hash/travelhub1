...
## ROUND 1 — DOMAIN MODEL / ENTITY COVERAGE / VISIBILITY / RBAC / AUDIT / UX CONTRACT
...
Do not assume every entity has a direct create form.

---

# 30. CREATE-FORM SEMANTICS

If a note is entered during creation, decide whether it becomes:

```text
first OperationalNote attached after entity creation
```

or:

```text
dedicated entity field
```

If using OperationalNote collection, avoid a separate duplicate `note` column unless justified.
...
```text
docs/prompts/PHASE_3_STEP_3.5_PLATFORM_CRM_OPERATIONAL_NOTES_COMMENTS_ARCHITECTURE_RECONCILIATION_REPORT.md
```
