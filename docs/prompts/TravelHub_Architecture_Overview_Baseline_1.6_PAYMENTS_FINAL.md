...
| Finance | Payment, PaymentTerms, ProviderFee, Refund, Invoice, Commission, CommissionAccrual, Settlement, Payout, LedgerTransaction, Currency, ExchangeRate, Tax, TaxRule |
...
### Marketplace payment & settlement model

Finance поддерживает три settlement modes:

``` text
SPLIT_AT_PAYMENT   Buyer → PSP → Partner share + TravelHub fee
PLATFORM_COLLECT   Buyer → PSP/platform → Settlement → Payout Partner
PARTNER_COLLECT    Buyer → Partner → CommissionAccrual → Partner pays TravelHub
```

`SPLIT_AT_PAYMENT` — предпочтительный режим, если PSP/country/currency capability это поддерживает. `Payment` и `Payout` — разные сущности и могут использовать разные rails. Buyer обычно платит картой/wallet через PSP; Partner payout может идти банковским переводом.

`PaymentTerms`: FULL_PREPAYMENT, PARTIAL_PREPAYMENT, DEPOSIT, PAY_LATER, PAY_AT_SERVICE. Partner выбирает только разрешенные платформой terms; они фиксируются в Sale/Order snapshot.

`ProviderFee` отделен от TravelHub `Commission`. `Settlement` фиксирует gross/refunds/commission/provider fees/tax/adjustments/partner payable. `LedgerTransaction` хранит immutable financial movements.

Finance domain provider-agnostic: Stripe/Adyen/Mangopay/Checkout.com/Rapyd/bank rails реализуются adapters, а не domain entities.
