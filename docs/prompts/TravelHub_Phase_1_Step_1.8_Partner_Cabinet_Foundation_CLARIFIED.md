...
## 8.1 Partner-safe ACTIVE Category Schema contract — ОБЯЗАТЕЛЬНО

Для dynamic Product form PARTNER **не использует** internal endpoint `/category-schemas` и **не получает** internal permission:

```text
catalog.category_schema.read
```

Использовать только Partner-safe contract:

```text
GET /api/v1/partner/categories/:slug/schema
```

с отдельным permission:

```text
catalog.category_schema.read_active_for_product_edit
```

Contract возвращает только ACTIVE schema активной категории и editor-safe projection:
...
Dynamic editor в `/partner/products/new` и `/partner/products/:id/edit` должен получать Category Schema только через этот endpoint.

Нельзя:

- обращаться из Partner Cabinet к internal `/category-schemas`;
- давать PARTNER `catalog.category_schema.read`;
- читать DRAFT/DEPRECATED schema;
- принимать `version`/`schemaId` для обхода ACTIVE-only contract;
- отдавать internal audit/admin metadata;
- использовать Product ownership как prerequisite для чтения schema до создания Product.

Backend остаётся authoritative source validation.
...
7. dynamic Category Schema form использует только `/api/v1/partner/categories/:slug/schema`;
7a. Partner Cabinet не вызывает internal `/category-schemas`;
7b. PARTNER не имеет `catalog.category_schema.read`;
7c. PARTNER использует `catalog.category_schema.read_active_for_product_edit`;
8. string/text;
9. number/integer;
10. boolean;
11. date;
