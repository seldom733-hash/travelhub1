...
ARCHITECTURE AUDIT: VERDICT A — AUDIT COMPLETE (2026-08-29; commit `235d39d`; report — `PHASE_3_STEP_3.7_COMMUNICATION_INTEGRATION_ARCHITECTURE_REPOSITORY_GAP_AUDIT_REPORT.md`).

· **Step 3.7A --- Marketplace Contact Policy Authority** ✅ COMPLETE\
(2026-08-29; VERDICT A — server-side Marketplace Basic contact restriction;\
getCrmTier() conditional Prisma select omits email/phone for BASIC;\
frontend hides email column/cards for Basic;\
commit `271fbe3`; report — `PHASE_3_STEP_3.7A_MARKETPLACE_CONTACT_POLICY_IMPLEMENTATION_REPORT.md`).

· **Step 3.7A.1 --- Marketplace Contact Policy Runtime/Security/Entitlement Evidence** ✅ COMPLETE\
(2026-08-29; VERDICT A — Basic tier=200, Pro tier=200 with email/phone,\
cross-partner→404, Platform CRM unaffected, 106/106 CRM + 65/65 analytics + 243/243 frontend PASS;\
commit `2c5b202`; report — `PHASE_3_STEP_3.7A.1_PARTNER_CRM_CONTACT_POLICY_EVIDENCE_CLOSURE_REPORT.md`).

· **Step 3.7A.2 --- Final Runtime Evidence Closure** ✅ COMPLETE\
(2026-08-29; VERDICT A — BASIC non-empty list/detail proven with fixture (email/phone ABSENT);\
entitlement fallback (EXPIRED storefront → BASIC + restricted payload);\
chat anti-disintermediation CONFIRMED WORKING (email→422, phone→422, URL→422);\
correction: prior 3.7A.1 incorrectly stated "not chat-level" — catalog + chat both use\
shared anti-disintermediation.ts regex; 126/126 CRM+Communication PASS;\
commit `c543ab8`; report — `PHASE_3_STEP_3.7A.2_FINAL_RUNTIME_EVIDENCE_CLOSURE_REPORT.md`).
...
· **Step 3.10 --- Support Domain** ✅ STRICT REVIEW RE-QUALIFICATION APPROVED — CLOSED (2026-08-30; VERDICT A — Support Domain; Ticket/Case lifecycle, priority, SLA, assignment, escalation, comments, communication links, audit history; canonical references to Customer/Order/Booking (no FK, ADR-0001); implementation 7d638ef → Strict Review ff64a83 — VERDICT B (F1 P1: RBAC RolePermission rows missing, F2 P2: getCase internal comment disclosure, F3 P2: createCase/assignCase no related entity validation, F4 P3: escalateCase bypasses VALID_TRANSITIONS, F5 P3: linkCommunication no communicationId validation) → findings remediation bb53fb0 (F1: idempotent migration seed support.case.* RolePermission rows for ADMIN/OPERATOR/DIRECTOR/FINANCE/ANALYST/SALES_MANAGER; F2: server-authoritative isInternal comment filtering; F3: assertEntityExists for customer/order/booking/assignee + ineligible role rejection; F4: escalateCase delegates to canonical transitionCase; F5: communicationId existence validation) → Re-Qualification VERDICT A — F1–F5 CLOSED — STEP 3.10 CLOSED; support suite 30/30 + communication suite 44/44 + backend TSC PASS.
...
## Prisma/Runtime Schema Drift & Auth Login Remediation (Phase 3 Pre-Step 3.12)

Реализовано и верифицировано (SHA: pending).

### Root Cause

Runtime database (`travelhub1`) уже содержал `security.User` с колонкой `username`, но:
- Другая копия БД (`travelhub`) не имела security schema.
- Авторские пароли не совпадали с ожиданиями (mismatch в bcrypt hash).
- Admin password сброшен через прямой UPDATE.

### Resolution

1. Runtime DB (`travelhub1`): `security.User` с 70 пользователями — **уже существует**.
2. Admin password reset: `bcrypt.hash('Admin123!')` → DB update.
3. Auth API: `POST /auth/login` → `accessToken` + HttpOnly cookie → **PASS**.
4. Browser login: `type()` с delay для React state → cookie set → redirect → **PASS**.
5. Route smoke test: dashboard, command-center, analytics, orders, bookings, payments — **ALL OK**.

### Evidence

- Valid login: `username=admin, password=Admin123!` → token + ADMIN role + 151 permissions ✅
- Invalid password → 401 Unauthorized ✅
- Unknown user → 401 Unauthorized ✅
- Browser login → `/app/dashboard` ✅
...
## Marketplace / Storefront Tenant-Scoped Reference Number Contract — Final Strict Review & Re-Qualification (Phase 3 Pre-Step 3.12)

Финальный Strict Review подтвердил квалификацию Reference Number Contract.

### Canonical Format Reconciliation

| Entity | Runtime Format | Backfill Format | Canonical |
|---|---|---|---|
| Order (Marketplace) | `MKT-ORD-{SEQ6}` | `MKT-ORD-{ROW_NUMBER}` | ✅ |
| Order (Storefront) | `{SF_CODE}-ORD-{SEQ6}` | `SF000-ORD-{ROW_NUMBER}` | ✅ (quarantine) |
| Booking | `MKT-BKG-{SEQ6}` / `{SF_CODE}-BKG-{SEQ6}` | Same pattern | ✅ |
| Payment | `MKT-PAY-{SEQ6}` / `{SF_CODE}-PAY-{SEQ6}` | Same pattern | ✅ |
| Refund | `MKT-REF-{SEQ6}` / `{SF_CODE}-REF-{SEQ6}` | Same pattern | ✅ |
| StorefrontCode | `SF001`, `SF002`, ... | Same pattern | ✅ |

### StorefrontCode Allocator — DEFECT REMEDIATED

**Defect found**: `storefront.service.ts` использовал `count()+1` (concurrency hazard).
**Fix**: заменён на Hi/Lo block allocation через `IdsService.nextStorefrontCode()`.

### DB Constraints

- `Order.referenceNumber` — `@unique` + unique index ✅
- `Booking.referenceNumber` — `@unique` + unique index ✅
- `Payment.referenceNumber` — `@unique` + unique index ✅
- `Refund.referenceNumber` — `@unique` + unique index ✅
- `PartnerStorefront.storefrontCode` — `@unique` + unique index ✅

### Concurrency Evidence

- 20 concurrent MKT-ORD allocations → 0 duplicates ✅
- 20 concurrent SF001-ORD allocations → 0 duplicates ✅
- Cross-tenant concurrent allocation → 0 cross-tenant duplicates ✅
- Cross-type concurrent allocation → 0 cross-type duplicates ✅
- 4 tenants × 2 types → 0 duplicates across 200 allocations ✅

### SF000 Quarantine Status

- 94 Orders, 51 Bookings, 67 Payments, 20 Refunds with SF000 prefix
- SF000 = unresolved provenance (quarantine)
- No SF000 storefront code exists (no fake tenant) ✅
- Ownership not fabricated ✅

### Immutability

- `referenceNumber`: set once at creation, never exposed in update DTOs ✅
- `storefrontCode`: set once at creation, excluded from forbidden keys ✅
- No API/PATCH endpoint mutates these fields ✅

### Tenant Isolation

- PARTNER role: only `storefront.*_own` permissions (no order/booking/payment/refund) ✅
- ADMIN role: `order.read`, `booking.read` (no payment/refund read) ✅
- `acquisitionSource` filter in `listOrders` defaults to MARKETPLACE ✅
- No prefix-based authorization (RBAC enforced via PermissionsGuard) ✅

### Representative Data (runtime DB `travelhub1`)

| Entity | Total | Marketplace | Storefront | SF000 Quarantine | NULL |
|---|---|---|---|---|---|
| Orders | 1516 | 1085 | 337 | 94 | 0 |
| Bookings | 692 | 405 | 236 | 51 | 0 |
| Payments | 816 | 484 | 265 | 67 | 0 |
| Refunds | 334 | 188 | 126 | 20 | 0 |
| Storefronts | 13 | — | 13 | — | — |
| Partners | 28 | — | — | — | — |

### Cross-Entity Integrity

- All Bookings reference valid Orders ✅
- All Payments reference valid Orders ✅
- Zero duplicate referenceNumbers across all entities ✅

### Search / Lookup

- `GET /api/v1/orders?search=MKT-ORD-000001` → found (authoritative) ✅

### VERDICT A — REFERENCE NUMBER CONTRACT QUALIFIED

SHA: `cf1f1a2` (storefrontCode allocator fix)
...
| Step 3.7 — Communication Integration Audit | ✅ DONE (committed 235d39d) |
...
37. **PHASE 3 — STEP 3.7 — COMMUNICATION INTEGRATION AUDIT** ✅ COMPLETE (2026-08-29; VERDICT A — architecture/repository gap audit; communication schema, anti-disintermediation, reverse-conversation, tenant isolation inventoried; commit `235d39d`).
