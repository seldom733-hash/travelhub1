...
**Дата актуализации:** 2026-08-18 (Step 2.17C ✅ IMPLEMENTATION COMPLETED — BEHAVIOR-PRESERVING DECOMPOSITION; Service Templates decision gates DD-024…DD-029 RESOLVED; Universal Pricing Model Amendment INTEGRATED — docs/architecture/universal-pricing-model.md; Canonical Roadmap Gap & Status Reconciliation Audit COMPLETED 2026-08-12 — статусы 1.12.3/1.18/1.18A/2.0/2.6 синхронизированы; Step 2.7 ✅ STRICT REVIEW COMPLETED — APPROVED WITH REVIEW FIXES; Step 2.8 ✅ STRICT REVIEW COMPLETED — APPROVED; Step 2.8A ✅ STRICT REVIEW COMPLETED — APPROVED; Step 2.9 ◀ IMPLEMENTATION COMPLETED — WAITING FOR STRICT REVIEW (2026-08-13))\
...
· **Step 2.17C --- Sales Domain Structural Decomposition** ✅ IMPLEMENTATION COMPLETED — BEHAVIOR-PRESERVING DECOMPOSITION (2026-08-18, Wave 0–6, facade 2,527 → 440 lines) (2026-08-18; отчёт — `docs/prompts/PHASE_2_STEP_2.17C_SALES_STRUCTURAL_DEBT_DESIGN_AND_DECOMPOSITION_REPORT.md`; дизайн — `docs/architecture/sales-structural-decomposition-2.17C.md`; repository-first VERDICT A — DESIGN READY FOR IMPLEMENTATION; baseline `f688f57`: `sales.service.ts` 2,527 lines / 41 public async / 25 private / 22 tx roots / 6 deps / 1 event (OrderRequested) / sole-writer invariant clean (ADR-0001), 0 duplicate authority; inventory 66/66 methods; decomposition: SalesService = stable facade (Option A, сохраняет Reverse in-tx `createOpportunityFromBuyerRequestSelection(tx,…)` boundary verbatim) + 4 collaborator services (SalesQueryService / SalesLifecycleService / SalesQuoteService / SalesCheckoutService / SalesCompletionService) + 2 pure modules (sales.projection.ts — 8 DTO mappers; sales.history.ts); REJECTED: SalesPricingSnapshotService (уже pure в sales.money), SalesOwnershipService (нет ownership-предикатов), SalesSequenceService (IdsService = platform infra), SalesEventService (1 событие/1 сайт); MERGED: SalesHistoryService → module; DEFERRED: SalesCommissionService; 0 authority changes, 0 transaction/event/idempotency/money/status/RBAC changes, dependency graph acyclic (0 cycles), completeSale single-tx atomicity (CAS+snapshot+Catalog reserve+outbox emit, post-commit publishEvent) сохраняется в SalesCompletionService (Wave 5, last); implementation waves Wave 0–6 (Wave 0 = characterization: golden projection tests + unit pinning issueQuote/completeSale; каждая волна buildable/testable/revertible); characterization: критичное поведение сильно закреплено e2e (1194 serial e2e green, 10 sales-спецификаций + ~15 specs с sales-endpoints), gaps unit-level → Wave 0; boundaries: 2.17B BLOCKED unchanged, payment branch unchanged, RLS unchanged, 2.18 NOT STARTED, Phase 2 exit blocked; negative checks все 0 (0 production/schema/migrations/API/RBAC/events/idempotency/money/status/perf changes); artifact integrity PASS=151 WARN=0 FAIL=0 (checker regression 13/13); implementation НЕ начата — NEXT = STEP 2.17C BEHAVIOR-PRESERVING IMPLEMENTATION; Step НЕ помечен APPROVED / НЕ IMPLEMENTATION COMPLETED; Step 2.17B остаётся BLOCKED — FINAL QUALIFICATION ENVIRONMENT REQUIRED — NOT APPROVED; PHASE 2 EXIT остаётся BLOCKED). Предшествующий (2026-08-15 ownership reconciliation — `docs/prompts/TRAVELHUB_STEP_2.17C_SALES_STRUCTURAL_DEBT_OWNERSHIP_AND_STEP_2.9_STATUS_RECONCILIATION_REPORT.md`; реализация НЕ начата; maintainability/structural-risk step, НЕ feature step)\
