# TravelHub --- Implementation Prompt Phase 3 — Baseline 1.6 PAYMENTS FINAL
...
## 1.1 Catalog / Marketplace foundation уже заморожен

Phase 3 завершает публичный Marketplace поверх Catalog foundation Phase 1–2 и не создает параллельную Product-модель.

Обязательные invariants:

- единый `Catalog.Product` для всех категорий туристических услуг;
- category schema определяет category-specific attributes, filters, availability, Tariff/Option, media requirements и секции Product Detail Page;
- Product media record/asset принадлежит Catalog; отдельный media domain не создается;
- PARTNER управляет только собственными draft Product/media;
- MODERATOR выполняет review/approve/reject/request changes, но не редактирует Product/media за PARTNER;
- public API читает только published Product/media;
- существенное изменение опубликованного Product/media не заменяет публичную версию до нового разрешенного moderation/publish transition;
- `/` остается Public Marketplace, а `/app/*` — внутренним контуром;
- Product Detail Page и media gallery обязательны.

Если фактическая Phase 1/2 реализация этим правилам не соответствует, сначала подготовь migration/refactor plan. Не обходи конфликт созданием второго Catalog.
...
## Finance UI dependency — Baseline 1.6

Phase 3 Finance/Partner/Buyer UI must expose the Phase 2 financial model without reimplementing calculations: PaymentTerms, Payment, ProviderFee, Commission, Settlement, Payout, CommissionAccrual and ledger-backed reconciliation.
