...
export class AzalHttpService implements OnModuleDestroy {
  ...
  private lastSearchQuery: AzalFlightSearchQuery | null = null;
  ...
  async histograms(
    ...
    return this.withPage(async (page) => {
      ...
      let nativeHistogramResponse: unknown = null;
      let nativeHistogramStatus: number | null = null;

      const onRequest = (req: import("playwright").Request) => {
        if (!req.url().includes(AzalHttpService.HISTOGRAM_PATH)) return;
        ...
        const entry = {
          ...
          url: req.url(),
          headers: {
            "x-auth-token": headers["x-auth-token"] ?? null,
            "x-conversation": headers["x-conversation"] ?? null,
            "x-application": headers["x-application"] ?? null,
            "x-client-id": headers["x-client-id"] ?? null,
            "x-locale": headers["x-locale"] ?? null,
        ...
        this.logger.log(
          `[AZAL HISTOGRAM REQUEST] ${JSON.stringify(entry)}`,
        );
      };

      const onResponse = async (response: import("playwright").Response) => {
        if (!response.url().includes(AzalHttpService.HISTOGRAM_PATH)) return;

        nativeHistogramStatus = response.status();
        ...
        diagnostics.push(entry);
        this.logger.log(
          `[AZAL HISTOGRAM RESPONSE] ${JSON.stringify(entry)}`,
        );
      };
      ...
      try {
        /*
         * Diagnostic only:
         * open the real AZAL /book/select browser page in the SAME session
         * created by Search. We do not synthesize another API request and
         * we do not hardcode expired x-auth-token/x-conversation values.
         */
        if (this.lastSearchQuery) {
          const q = this.lastSearchQuery;
          ...
          const params = new URLSearchParams({
            ...
            tripType: q.tripType,
            adult_count: String(q.adults),
            child_count: String(q.children ?? 0),
            infant_count: String(q.infants ?? 0),
            is_student: q.isStudent ? "1" : "0",
            timestamp: String(Date.now()),
          });

          const selectUrl =
            `${AzalHttpService.BASE_URL}/book/select?${params.toString()}`;

          this.logger.log(
            `[AZAL HISTOGRAM DIAGNOSTIC] opening ${selectUrl}`,
          );
          ...
          // Allow the actual AZAL frontend to initialise and emit requests.
        ...
        return {
          ...
          raw: {
            diagnostic: true,
            nativeHistogramStatus,
            nativeHistogramResponse,
            capturedRequests: diagnostics,
            session: {
              hasAuthToken: Boolean(this.authToken),
              hasConversation: Boolean(this.conversation),
            },
            message:
              "Diagnostic mode: no synthetic histogram API request was sent.",
          },
        };
      } finally {
