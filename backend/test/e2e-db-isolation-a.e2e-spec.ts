/**
 * Round 6 — DB Isolation Contract Suite A
 *
 * Proves per-suite PostgreSQL database isolation:
 * - current_database() matches E2E_SUITE_DB_NAME
 * - current_database() is NOT the shared base travelhub1_test
 * - Sentinel table is isolated: no Suite B data present
 *
...
describe("Round 6 — DB Isolation Contract Suite A", () => {
  ...
  it("current_database() matches E2E_SUITE_DB_NAME", async () => {
    ...
    console.log(`[isolation-a] current_database = ${dbName}, expected = ${expectedName}`);
  });
  ...
  it("creates sentinel table and inserts Suite A marker", async () => {
    ...
    // Verify no Suite B sentinel exists (Suite B has NOT run yet in A→B order)
    const bRows = await prisma.$queryRawUnsafe<{ key: string }[]>(
      "SELECT key FROM public.e2e_isolation_sentinel WHERE key = $1",
      "round6-suite-b",
    );
    expect(bRows).toHaveLength(0);

    // Insert Suite A sentinel
    await prisma.$executeRawUnsafe(
    ...
    // Verify only Suite A sentinel exists
    ...
    expect(allRows[0].key).toBe("round6-suite-a");
    expect(allRows[0].value).toBe("sentinel-value-a");
  });
});
