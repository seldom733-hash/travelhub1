...
export async function getCrmData(f: AnalyticsFilters): Promise<AnalyticsSectionData> {
  ...
  return {
    ...
    kpis: [
      { key: "total", title: "Всего клиентов", value: totalBuyers, unit: " чел.", tone: "neutral" },
      { key: "new", title: "Новые клиенты", value: newBuyers, unit: " чел.", change: changePct(newBuyers, prevNewBuyers), spark: growthSeries.values, tone: "neutral" },
      { key: "active", title: "Активные (покупки)", value: activeBuyers, unit: " чел.", tone: "positive" },
      { key: "repeat", title: "Повторные клиенты", value: repeatPct, unit: "%", tone: repeatPct >= 30 ? "positive" : "neutral" },
      { key: "vip", title: "VIP-клиенты", value: vipUsers, unit: " чел.", tone: "positive" },
      { key: "ltv", title: "Средний LTV", value: Math.round(ltv) },
      { key: "avgCheck", title: "Средний чек", value: Math.round(avgCheck) },
      { key: "ordersPerUser", title: "Покупок на клиента", value: Math.round(avgOrdersPerUser * 10) / 10, unit: " шт", tone: "neutral" },
